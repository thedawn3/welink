...
const isSourceStandard = (directory?: DirectoryValidation) => {
  ...
  if (
    directory.has_contact === undefined ||
    directory.has_message === undefined
  )
    return undefined;
...
export const SystemRuntimeView: React.FC<SystemRuntimeViewProps> = ({
  ...
  const filteredLogs = useMemo(() => {
    return logs.filter((entry) => {
      const sourceOk = logSourceFilter
        ? entry.source === logSourceFilter
        : true;
      const levelOk = logLevelFilter
        ? (entry.level || "")
            .toLowerCase()
            .includes(logLevelFilter.toLowerCase())
        : true;
  ...
  const startButtonLabel = isDockerManualMode
    ? "校验并同步标准目录"
    : "启动解密";
  const canStartDecrypt = configCheck
    ? Boolean(configCheck.can_start_sync)
  ...
  const configWarnings = useMemo(() => {
    ...
    return flattenMessages(
      ...
      configCheck.media?.warnings,
    );
  }, [configCheck]);

  const mediaPreviewLabel = useMemo(() => {
    switch ((configCheck?.media?.preview_state || "").toLowerCase()) {
      case "ready":
        return "已就绪";
      case "partial":
        return "部分可用";
      case "disabled":
        return "未启用";
      default:
        return valueOrDash(configCheck?.media?.preview_state);
    }
  }, [configCheck?.media?.preview_state]);

  const sourceConfigured = Boolean(
    (configCheck?.source_dir?.path || "").trim(),
  );
  ...
  const stopDisabled = !["running", "stopping"].includes(
    String(mergedStatus.decrypt_state || "").toLowerCase(),
  );
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5 mb-6">
        <div className="flex flex-wrap items-center gap-2 mb-3">
          ...
          <span className="text-[11px] text-gray-600 bg-gray-100 px-2 py-1 rounded-full">
            部署：
            {valueOrDash(
              configCheck?.deployment_target || runtime?.deployment_target,
            )}
          </span>
          <span className="text-[11px] text-gray-600 bg-gray-100 px-2 py-1 rounded-full">
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          <div className="rounded-xl border border-gray-100 p-3">
            ...
            <p className="text-[11px] text-gray-500 mt-1 break-all">
              {valueOrDash(configCheck?.source_dir?.path)}
            </p>
            <p className="text-[11px] text-gray-600 mt-2">
              存在：{boolLabel(configCheck?.source_dir?.exists)}
            </p>
            <p className="text-[11px] text-gray-600">
              标准目录：{boolLabel(isSourceStandard(configCheck?.source_dir))}
            </p>
            <p className="text-[11px] text-gray-600">
              包含 contact：{boolLabel(configCheck?.source_dir?.has_contact)}
            </p>
            <p className="text-[11px] text-gray-600">
              包含 message：{boolLabel(configCheck?.source_dir?.has_message)}
            </p>
            <p className="text-[11px] text-gray-600">
              包含 sns：{boolLabel(configCheck?.source_dir?.has_sns)}
            </p>
          </div>
          <div className="rounded-xl border border-gray-100 p-3">
            <p className="text-xs font-semibold text-gray-500">Analysis 目录</p>
            <p className="text-[11px] text-gray-500 mt-1 break-all">
              {valueOrDash(configCheck?.analysis_dir?.path)}
            </p>
            <p className="text-[11px] text-gray-600 mt-2">
              存在：{boolLabel(configCheck?.analysis_dir?.exists)}
            </p>
            <p className="text-[11px] text-gray-600">
              包含 contact：{boolLabel(configCheck?.analysis_dir?.has_contact)}
            </p>
            <p className="text-[11px] text-gray-600">
              包含 message：{boolLabel(configCheck?.analysis_dir?.has_message)}
            </p>
          </div>
          <div className="rounded-xl border border-gray-100 p-3">
            <p className="text-xs font-semibold text-gray-500">
            ...
            <p className="text-[11px] text-gray-500 mt-1 break-all">
              {valueOrDash(configCheck?.work_dir?.path)}
            </p>
            <p className="text-[11px] text-gray-600 mt-2">
              work_dir 可写：{boolLabel(configCheck?.work_dir?.writable)}
            </p>
            <p className="text-[11px] text-gray-600">
              decrypt 支持：{boolLabel(configCheck?.decrypt?.supported)}
            </p>
            <p className="text-[11px] text-gray-600">
              sync 支持：{boolLabel(configCheck?.sync?.supported)}
            </p>
            <p className="text-[11px] text-gray-600">
              sns 就绪：{boolLabel(configCheck?.sns?.ready)}
            </p>
            <p className="text-[11px] text-gray-500 break-all">
              sns.db：{valueOrDash(configCheck?.sns?.db_path)}
            </p>
          </div>
          <div className="rounded-xl border border-gray-100 p-3">
            <p className="text-xs font-semibold text-gray-500">
            ...
            <p className="text-[11px] text-gray-500 mt-1 break-all">
              {valueOrDash(configCheck?.media?.path)}
            </p>
            <p className="text-[11px] text-gray-500 break-all">
              wechat-decrypt 目录：
              {valueOrDash(configCheck?.media?.wechat_decrypt_dir)}
            </p>
            <p className="text-[11px] text-gray-600 mt-2">
              图片预览：{boolLabel(configCheck?.media?.image_preview_enabled)}
            </p>
            <p className="text-[11px] text-gray-600">
              预览状态：{mediaPreviewLabel}
            </p>
            <p className="text-[11px] text-gray-600">
              检测到 V2 图片：
              {boolLabel(configCheck?.media?.v2_images_detected)}
            </p>
            <p className="text-[11px] text-gray-600">
              已配置图片密钥：
              {boolLabel(configCheck?.media?.image_aes_key_configured)}
            </p>
            <p className="text-[11px] text-gray-500 break-all">
              密钥来源：{valueOrDash(configCheck?.media?.image_aes_key_source)}
            </p>
            <p className="text-[11px] text-gray-600">
              密钥模式：{valueOrDash(configCheck?.media?.image_key_mode)}
            </p>
            <p className="text-[11px] text-gray-600">
              已加载密钥数：
              {valueOrDash(configCheck?.media?.image_key_count)}
            </p>
            {configCheck?.media?.suggested_command ? (
              <div className="mt-2 rounded-lg bg-gray-50 px-2 py-2">
                <p className="text-[10px] font-semibold text-gray-500">
                  推荐宿主机命令
                </p>
                <code className="mt-1 block break-all text-[10px] text-gray-700">
                  {configCheck.media.suggested_command}
                </code>
              </div>
            ) : null}
            {configCheck?.media?.issues &&
              configCheck.media.issues.length > 0 && (
                <p className="text-[11px] text-red-500 mt-2">
                  {configCheck.media.issues[0]}
                </p>
              )}
            {configCheck?.media?.warnings &&
              configCheck.media.warnings.length > 0 && (
                <p className="text-[11px] text-amber-600 mt-2">
                  {configCheck.media.warnings[0]}
                </p>
              )}
          </div>
        </div>
        {isAnalysisOnlyReady ? (
          <div className="mt-3 rounded-xl border border-sky-200 bg-sky-50 px-3 py-2">
            ...
            <p className="text-xs text-sky-700 mt-1">
              analysis
              目录已就绪，可直接浏览数据并按需强制重建索引；如需手动同步，请额外配置标准
              source 目录。
            </p>
          </div>
        ) : (
        ...
        {configCheck?.suggested_actions &&
          configCheck.suggested_actions.length > 0 && (
            <div className="mt-3">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5 mb-6">
        ...
        {(mergedStatus.last_change_reason ||
          ...
          <p className="text-xs text-gray-500 mt-2 flex flex-wrap gap-4">
            {mergedStatus.last_change_reason && (
              <span>
                最新变更原因：{valueOrDash(mergedStatus.last_change_reason)}
              </span>
            )}
            {runtime?.updated_at && (
              <span>
            ...
            {runtime?.last_message_at && (
              <span>
                最近消息时间：
                {new Date(runtime.last_message_at).toLocaleString()}
              </span>
            )}
            {runtime?.last_sns_at && (
              <span>
                最近朋友圈时间：{new Date(runtime.last_sns_at).toLocaleString()}
              </span>
            )}
          </p>
        )}
        {actionNotice && (
          <p className="text-xs text-[#07c160] mt-2">{actionNotice}</p>
        )}
        {error && <p className="text-xs text-red-500 mt-2">{error}</p>}
        {loading && (
          <p className="text-xs text-gray-400 mt-2">正在同步运行时状态...</p>
        )}
        {meta?.pollingFallback && (
          ...
          {isDockerManualMode ? (
            <div className="md:col-span-2 lg:col-span-3 rounded-xl border border-gray-100 bg-gray-50 px-3 py-2">
              <p className="text-xs text-gray-600">
                当前为 Docker 手动同步模式：`auto_refresh` 与 `wal`
                已按系统策略固定，不建议在容器内启用 watcher。
              </p>
            </div>
          ) : (
            <>
              ...
              tasks.map((task, idx) => (
                ...
                >
                  ...
                  <p className="text-xs text-gray-400">
                    {valueOrDash(task.message || task.detail)}
                  </p>
                  {task.error && (
          ...
          <div className="space-y-2 max-h-56 overflow-auto">
            {filteredLogs.length === 0 ? (
              ...
              filteredLogs.map((entry, idx) => (
                <div
                  key={`${entry.time ?? idx}-${idx}`}
                  className="rounded-xl border border-gray-100 px-3 py-2"
                >
                  <p className="text-[11px] text-gray-500">
                    #{valueOrDash(entry.id)} · {valueOrDash(entry.time)} ·{" "}
                    {valueOrDash(entry.level)} · {valueOrDash(entry.source)}
                  </p>
                  <p className="text-xs text-[#1d1d1f] break-words">
                    {entry.message}
                  </p>
                  {entry.fields && (
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <div className="rounded-xl border border-gray-100 p-3">
            ...
            <input
              ...
              placeholder={
                defaultContactUsername
                  ? `联系人 username（默认 ${defaultContactUsername}）`
                  : "联系人 username"
              }
            ...
            <button
              onClick={() =>
                onExportContact(contactTarget, contactLimit || undefined)
              }
          ...
          <div className="rounded-xl border border-gray-100 p-3">
            ...
            <input
              ...
              placeholder={
                defaultGroupUsername
                  ? `群聊 username（默认 ${defaultGroupUsername}）`
                  : "群聊 username"
              }
              className="w-full px-3 py-2 rounded-lg border border-gray-200 text-xs focus:outline-none focus:ring-2 focus:ring-[#07c160]/20"
            />
